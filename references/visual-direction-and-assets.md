...
每个非证据型语义窗都要明确回答“生成一张情境图是否能让观众更快看懂或更愿意继续看”。开头、情绪转换、抽象痛点和相邻画面过于相似时尤其要主动评估；可生成办公室任务堆积、人物使用情境、文件流转空间或抽象隐喻，再叠加确定性的官方 Logo 与真实产品界面。生成图不得承载产品身份、按钮、参数、执行结果或事实证据。
...
- 证据标注必须落到当前口播真正提到的词、按钮、字段、表格行或结果数字；不能因为旁白概括了一段内容，就用一个大圈笼统包住整段截图。
- 标注不是盖在目标上的涂鸦层。先取得目标实际可见像素外接框，再把圈线或括号向外扩；箭头落在目标外缘，整条箭杆避开正文、图标、按钮与结果。长句优先用圆角外框或四角括号，空间不足先扩大证据区或换标注形式，不得让圈线穿字。
- 圈、圆角框和括号的大小必须逐目标从实际 `contentBounds` 计算，并加入清楚的外扩间隙；禁止固定尺寸批量套用。箭头只指一个目标，优先短直线或一次规整折线，不能跨卡片、穿正文、压图标、与其他箭头交叉或绕出混乱轨迹。否定状态用独立状态胶囊或图标表示，不用斜线穿过原文。
- 动态路径必须从“有意义的第一段”开始可见。采用圆头线帽时，零进度和近零进度的箭头、勾号、下划线、连接线不得渲染，避免出现先闪一个点、再长成线的机械瑕疵；目标节点先稳定，再显示达到最小可读长度的路径。
- 来源截图若自带页码压标题、浮层遮字、原文截断、暗屏或无关阴影，必须先改裁切、重排证据或在不改变事实的前提下重绘受损标题。不得用整块暗幕、黑色阴影或降低整图可读性来制造聚焦。
- 同一语义窗连续讲两个可选项、两个范围或两个结果时，可以用上下堆叠的两个来源区域接力；每个区域分别保留页面身份和必要上下文，分别绑定发声锚点，不能把两处裁成两个孤立残句。
...
- 相邻语义窗必须在背景、构图、主对象尺度、证据类型或运动语法中至少有一项明显变化。上一屏会干扰新语义时要完整退出，禁止把新内容半透明地盖在上一屏上。只有系列头部、进度条等稳定导航可以连续保留；
...
长片、官网证据讲解和已经发生过遮挡、裁切、闪帧、重复或 MG 缺失的项目，还必须执行 [rendered-frame-and-audio-audit.md](rendered-frame-and-audio-audit.md) 的逐锚点与边界终审。
